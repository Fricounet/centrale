...
module.exports.handle = async event => {
    const userId = "zubiate_wesley" //event.pathParameters.id;
    ...
    // Get movies uuid (to make sure we do not suggest an already rated movie)
    const moviesUuid = [];
    ...
    // console.log(results)
    // const preferedRatings = results.Items.filter(item => {
    //     return parseInt(item.rating) > 3 && item.uuid.includes(userId)
    // })
    // console.log(preferedRatings)
